...
import { Dt as resolveControlUiLinks, Et as randomToken, Gg as writeConfigFile, Vg as readConfigFileSnapshot, ft as DEFAULT_WORKSPACE, gt as ensureWorkspaceAndSessions, jt as waitForGatewayReachable, pt as applyWizardMetadata, xt as normalizeGatewayTokenInput, yt as handleReset } from "./auth-profiles-iXW75sRj.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-BHHTMqXa.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-Cigis-qv.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-DYkJACkj.js").then((n) => n.i);
