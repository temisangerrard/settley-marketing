...
import { At as resolveControlUiLinks, C_ as readConfigFileSnapshot, D_ as writeConfigFile, Pt as waitForGatewayReachable, St as handleReset, gt as applyWizardMetadata, ht as DEFAULT_WORKSPACE, kt as randomToken, wt as normalizeGatewayTokenInput, yt as ensureWorkspaceAndSessions } from "./reply-BEN3KNDZ.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-CCVjRSH4.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-Ck-sMOU7.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-BnAAsjEg.js").then((n) => n.i);
